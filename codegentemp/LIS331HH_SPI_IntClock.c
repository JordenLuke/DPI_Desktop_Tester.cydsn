/*******************************************************************************
* File Name: LIS331HH_SPI_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    LIS331HH_SPI_IntClock_CLKEN |= LIS331HH_SPI_IntClock_CLKEN_MASK;
	LIS331HH_SPI_IntClock_CLKSTBY |= LIS331HH_SPI_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    LIS331HH_SPI_IntClock_CLKEN &= (uint8)(~LIS331HH_SPI_IntClock_CLKEN_MASK);
	LIS331HH_SPI_IntClock_CLKSTBY &= (uint8)(~LIS331HH_SPI_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_StopBlock(void) 
{
    if ((LIS331HH_SPI_IntClock_CLKEN & LIS331HH_SPI_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(LIS331HH_SPI_IntClock__CFG3)
        CLK_DIST_AMASK = LIS331HH_SPI_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LIS331HH_SPI_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LIS331HH_SPI_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LIS331HH_SPI_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LIS331HH_SPI_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        LIS331HH_SPI_IntClock_CLKSTBY &= (uint8)(~LIS331HH_SPI_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = LIS331HH_SPI_IntClock_GetSourceRegister();
    uint16 oldDivider = LIS331HH_SPI_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LIS331HH_SPI_IntClock_CLKEN & LIS331HH_SPI_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LIS331HH_SPI_IntClock_DIV_PTR, clkDivider);
                LIS331HH_SPI_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LIS331HH_SPI_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LIS331HH_SPI_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LIS331HH_SPI_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LIS331HH_SPI_IntClock_DIV_PTR, clkDivider);
				LIS331HH_SPI_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LIS331HH_SPI_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(LIS331HH_SPI_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    LIS331HH_SPI_IntClock_MOD_SRC |= modeBitMask & (uint8)LIS331HH_SPI_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    LIS331HH_SPI_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LIS331HH_SPI_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LIS331HH_SPI_IntClock_GetModeRegister(void) 
{
    return LIS331HH_SPI_IntClock_MOD_SRC & (uint8)(LIS331HH_SPI_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = LIS331HH_SPI_IntClock_GetDividerRegister();
    uint8 oldSrc = LIS331HH_SPI_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LIS331HH_SPI_IntClock_MOD_SRC |= CYCLK_SSS;
        LIS331HH_SPI_IntClock_MOD_SRC =
            (LIS331HH_SPI_IntClock_MOD_SRC & (uint8)(~LIS331HH_SPI_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LIS331HH_SPI_IntClock_GetSourceRegister(void) 
{
    return LIS331HH_SPI_IntClock_MOD_SRC & LIS331HH_SPI_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LIS331HH_SPI_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    LIS331HH_SPI_IntClock_PHASE = clkPhase & LIS331HH_SPI_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LIS331HH_SPI_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LIS331HH_SPI_IntClock_GetPhaseRegister(void) 
{
    return LIS331HH_SPI_IntClock_PHASE & LIS331HH_SPI_IntClock_PHASE_MASK;
}
