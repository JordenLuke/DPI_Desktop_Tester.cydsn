/*******************************************************************************
* File Name: TRIP_ACC_SPI_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_Start
********************************************************************************
*
* Summary:
...
void TRIP_ACC_SPI_IntClock_Start(void) 
{
    ...
    TRIP_ACC_SPI_IntClock_CLKEN |= TRIP_ACC_SPI_IntClock_CLKEN_MASK;
	TRIP_ACC_SPI_IntClock_CLKSTBY |= TRIP_ACC_SPI_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_Stop
********************************************************************************
*
* Summary:
...
void TRIP_ACC_SPI_IntClock_Stop(void) 
{
    ...
    TRIP_ACC_SPI_IntClock_CLKEN &= (uint8)(~TRIP_ACC_SPI_IntClock_CLKEN_MASK);
	TRIP_ACC_SPI_IntClock_CLKSTBY &= (uint8)(~TRIP_ACC_SPI_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((TRIP_ACC_SPI_IntClock_CLKEN & TRIP_ACC_SPI_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = TRIP_ACC_SPI_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = TRIP_ACC_SPI_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* TRIP_ACC_SPI_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(TRIP_ACC_SPI_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(TRIP_ACC_SPI_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = TRIP_ACC_SPI_IntClock_GetSourceRegister();
    uint16 oldDivider = TRIP_ACC_SPI_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = TRIP_ACC_SPI_IntClock_CLKEN & TRIP_ACC_SPI_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(TRIP_ACC_SPI_IntClock_DIV_PTR, clkDivider);
                TRIP_ACC_SPI_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                TRIP_ACC_SPI_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(TRIP_ACC_SPI_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((TRIP_ACC_SPI_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(TRIP_ACC_SPI_IntClock_DIV_PTR, clkDivider);
				TRIP_ACC_SPI_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(TRIP_ACC_SPI_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    TRIP_ACC_SPI_IntClock_MOD_SRC |= modeBitMask & (uint8)TRIP_ACC_SPI_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    TRIP_ACC_SPI_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(TRIP_ACC_SPI_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return TRIP_ACC_SPI_IntClock_MOD_SRC & (uint8)(TRIP_ACC_SPI_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = TRIP_ACC_SPI_IntClock_GetDividerRegister();
    uint8 oldSrc = TRIP_ACC_SPI_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        TRIP_ACC_SPI_IntClock_MOD_SRC |= CYCLK_SSS;
        TRIP_ACC_SPI_IntClock_MOD_SRC =
            (TRIP_ACC_SPI_IntClock_MOD_SRC & (uint8)(~TRIP_ACC_SPI_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return TRIP_ACC_SPI_IntClock_MOD_SRC & TRIP_ACC_SPI_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    TRIP_ACC_SPI_IntClock_PHASE = clkPhase & TRIP_ACC_SPI_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: TRIP_ACC_SPI_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return TRIP_ACC_SPI_IntClock_PHASE & TRIP_ACC_SPI_IntClock_PHASE_MASK;
}
