/*******************************************************************************
* File Name: HMC5883L_I2C_SLAVE.c
* Version 3.30
*
* Description:
...
volatile uint8 HMC5883L_I2C_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * HMC5883L_I2C_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   HMC5883L_I2C_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   HMC5883L_I2C_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * HMC5883L_I2C_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   HMC5883L_I2C_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   HMC5883L_I2C_slWrBufIndex; /* Slave Receive buffer Index */

#if(HMC5883L_I2C_SW_ADRR_DECODE)
    volatile uint8 HMC5883L_I2C_slAddress;  /* Software address variable */
#endif   /* (HMC5883L_I2C_SW_ADRR_DECODE) */


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  HMC5883L_I2C_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
uint8 HMC5883L_I2C_SlaveStatus(void) 
{
    return(HMC5883L_I2C_slStatus);
}


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveClearReadStatus
********************************************************************************
*
* Summary:
*  Clears the read status flags and returns they values.
*  The HMC5883L_I2C_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None
*
...
uint8 HMC5883L_I2C_SlaveClearReadStatus(void) 
{
    ...
    status = (HMC5883L_I2C_slStatus & HMC5883L_I2C_SSTAT_RD_MASK);
    HMC5883L_I2C_slStatus &= ((uint8) ~HMC5883L_I2C_SSTAT_RD_CLEAR);
...
/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveClearWriteStatus
********************************************************************************
*
* Summary:
*  Clears the write status flags and returns they values.
*  The HMC5883L_I2C_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None
*
...
uint8 HMC5883L_I2C_SlaveClearWriteStatus(void) 
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (HMC5883L_I2C_slStatus & HMC5883L_I2C_SSTAT_WR_MASK);
    HMC5883L_I2C_slStatus &= ((uint8) ~HMC5883L_I2C_SSTAT_WR_CLEAR);
...
/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveSetAddress
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  HMC5883L_I2C_Address  - used to store I2C slave address for the primary
...
{
    #if(HMC5883L_I2C_SW_ADRR_DECODE)
        HMC5883L_I2C_slAddress = (address & HMC5883L_I2C_SLAVE_ADDR_MASK);
    #else
        HMC5883L_I2C_ADDR_REG  = (address & HMC5883L_I2C_SLAVE_ADDR_MASK);
    #endif /* (HMC5883L_I2C_SW_ADRR_DECODE) */
}


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  HMC5883L_I2C_slRdBufPtr   - used to store pointer to slave read buffer.
*  HMC5883L_I2C_slRdBufSize  - used to store salve read buffer size.
     ...
     
{
    ...
    if(NULL != rdBuf)
    {
        HMC5883L_I2C_slRdBufPtr   = (volatile uint8 *) rdBuf;    /* Set buffer pointer */
        HMC5883L_I2C_slRdBufSize  = bufSize;    /* Set buffer size */
        HMC5883L_I2C_slRdBufIndex = 0u;         /* Clears buffer index */
    }
}


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  HMC5883L_I2C_slWrBufPtr   - used to store pointer to slave write buffer.
*  HMC5883L_I2C_slWrBufSize  - used to store salve write buffer size.
*  HMC5883L_I2C_slWrBufIndex - used to store current index within slave
*  write buffer.
*
* Side Effects:
*  If this function is called during a bus transaction, data from the previous
     ...
     
{
    ...
    if(NULL != wrBuf)
    {
        HMC5883L_I2C_slWrBufPtr   = (volatile uint8 *) wrBuf;  /* Set buffer pointer */
        HMC5883L_I2C_slWrBufSize  = bufSize;   /* Set buffer size */
        HMC5883L_I2C_slWrBufIndex = 0u;        /* Clears buffer index */
    }
}


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return(HMC5883L_I2C_slRdBufIndex);
}


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return(HMC5883L_I2C_slWrBufIndex);
}


/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HMC5883L_I2C_SlaveClearRxBuf
********************************************************************************
*
* Summary:
